...
namespace App.Web.Controllers
{
    [Audit]
    [Authorize]
    public class RadioTaxiController : Controller
    {
        protected readonly IGestionProcesos _repository;
        protected readonly ISIGPER _sigper;
        protected readonly IFile _file;

        public RadioTaxiController(IGestionProcesos repository, ISIGPER sigper, IFile file)
        {
        ...
        {
            var model = _repository.GetAll<RadioTaxi>();
            return View(model);
        }

        public ActionResult Details(int id)
        {
            var model = _repository.GetById<RadioTaxi>(id);
            return View(model);
        }
        ...
        public ActionResult Create(int WorkFlowId)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                model.Telefono = persona.Funcionario.Rh_TelOfi.Trim();
                model.Email = persona.Funcionario.Rh_Mail.Trim();
            }
        ...
        public ActionResult Create(RadioTaxi model)
        {
            model.Fecha = DateTime.Now;

            var dev = ExtensionesString.IsRut(model.Rut + "-" + model.DV);
            if (dev == false)
            {
                List<string> _err = new List<string>();
                string error = "Rut no es valido";
                _err.Add(error);
            ...
            else
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var _UseCaseResponseMessage = _useCaseInteractor.RadioTaxiInsert(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(RadioTaxi model)
        {
            if (ModelState.IsValid)
            {
                var _useCaseInteractor = new UseCaseInteractorCustom(_repository);
                var _UseCaseResponseMessage = _useCaseInteractor.RadioTaxiUpdate(model);
                if (_UseCaseResponseMessage.IsValid)
                {
                    TempData["Success"] = "Operación terminada correctamente.";
        ...
        //{
        //    var result = _repository.GetAll<RadioTaxi>();
