    ...
    public class GDExternoController : Controller
    {
        ...
        public ActionResult FEAUpload(DTOFileUploadFEA model)
        {
            ...
            if (model.PlazoEstablecido)
            {
                ...
                gd.PlazoEstablecido = model.PlazoEstablecido;
                gd.FechaPlazo = model.FechaPlazo;
                var _useCaseInteractor = new UseCaseGD(_repository, null, null, null);
                var _UseCaseResponseMessage = _useCaseInteractor.UpdatePlazo(gd);
                if (_UseCaseResponseMessage.IsValid)
                {
                    TempData["Success"] = "Operación terminada correctamente.";
                    return Redirect(Request.UrlReferrer.PathAndQuery);
        ...
        public FileResult ReportMinutaDocumentosOP(DTOReport model)
        {
            ...
            predicate = predicate.And(q => q.Fecha.Value.Year == model.Desde.Value.Year &&
            q.Fecha.Value.Month == model.Desde.Value.Month && q.Fecha.Value.Day == model.Desde.Value.Day);
