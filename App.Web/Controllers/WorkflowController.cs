...
namespace App.Web.Controllers
{
    ...
    public class WorkflowController : Controller
    {
        ...
        public ActionResult Send(Workflow model, int? Unidad, string Funcionario)
        {
            ...
            else if (workflow.DefinicionWorkflow.DefinicionProceso.Entidad.Codigo == Util.Enum.Entidad.DenunciaIntegridad.ToString())
            {
                var _useCaseInteractor = new UseCaseIntegridad(_repository, _sigper, _file, _folio, _hsm, _email);
                var _UseCaseResposeMessage = _useCaseInteractor.WorkflowUpdate(model);
                if (_UseCaseResposeMessage.IsValid)
                {
                    TempData["Success"] = "Operación terminada correctamente.";
                    return RedirectToAction("Index", "Workflow");
                }

                _UseCaseResposeMessage.Errors.ForEach(q => ModelState.AddModelError(string.Empty, q));
            }
            else
            {
